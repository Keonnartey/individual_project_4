...
def load(dataset="dbfs:/FileStore/databricks_project/top50MusicFrom2010-2019.csv"):
    ...
    new_column_names = [col.replace(' ', '_').replace(',', '').replace(';', '') for col in top50MusicFrom2010_2019_df.columns]
    top50MusicFrom2010_2019_df = top50MusicFrom2010_2019_df.toDF(*new_column_names)

    # Transform into a Delta Lakes table and store it 
    top50MusicFrom2010_2019_df.write.format("delta").mode("overwrite").saveAsTable("top50MusicFrom2010_2019_delta")
    ...
    protocol_version_row = history_df.filter("operation = 'WRITE'").select("operationParameters").first()

    if protocol_version_row:
        protocol_version_str = protocol_version_row[0]
        protocol_version = [param.split('=') for param in protocol_version_str.split(', ') if 'delta.protocol' in param]
        ...
        if protocol_version:
            ...
            if protocol_version != "2.5":
                spark.sql("ALTER TABLE top50MusicFrom2010_2019_delta SET TBLPROPERTIES ('delta.columnMapping.mode' = 'name', 'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5')")
