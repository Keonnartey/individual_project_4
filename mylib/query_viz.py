...
def query_transform():
    ...
    query = (
        'SELECT * FROM top50musicfrom2010_2019_delta '
...
# Show the result (you can also perform further actions like saving the result or displaying statistics)
result_df.show()
...
def simple_plot(df):
    ...
    plt.hist(
        ...
        bins=range(df.select("year").toPandas()["year"].min(), df.select("year").toPandas()["year"].max() + 1),
        color="green",
        alpha=0.7,
        edgecolor="black",
    )
